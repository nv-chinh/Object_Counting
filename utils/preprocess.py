...
def _shanghaitech(
    ...
    for split in ["train", "val"]:
        ...
        if split == "train":
            ...
            image_src_dir = os.path.join(data_src_dir, "val_data", "images")
            label_src_dir = os.path.join(data_src_dir, "val_data", "ground-truth")
            image_src_paths = glob(os.path.join(image_src_dir, "*.jpg"))
            label_src_paths = glob(os.path.join(label_src_dir, "*.mat"))
...
def _jhu(
    ...
    for split in ["train", "val"]:
        generate_npy = generate_npy and split == "train"
        image_src_paths = [os.path.join(data_src_dir, split, 'images', image_name) for image_name in os.listdir(os.path.join(data_src_dir, split, 'images'))]
        ...
        for i, image_src_path in tqdm(enumerate(image_src_paths), total=len(image_src_paths)):
            ...
            for label in labels:
                try:
                    cls,x,y,w,h = list(map(float, label.split(' ')))
                except:
...
def parse_args():
    ...
    parser.add_argument("--src_dir", type=str, default = "/home/chinhbrian/CLIP-EBC/Oil_Palm_Tree_Counting_Dataset", help="The root directory of the source dataset.")
    parser.add_argument("--dst_dir", type=str, default = "data/", help="The root directory of the destination dataset.")
