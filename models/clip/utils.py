...
def format_count(count: Union[float, Tuple[float, float]], prompt_type: str = "word") -> str:
    ...
    if count == 0:
        return "There is no cow." if prompt_type == "word" else "There is 0 cow."
    elif count == 1:
        return "There is one cow." if prompt_type == "word" else "There is 1 cow."
    elif isinstance(count, (int, float)):
        return f"There are {num2word(int(count))} cows." if prompt_type == "word" else f"There are {int(count)} cows."
    elif count[1] == float("inf"):
        return f"There are more than {num2word(int(count[0]))} cows." if prompt_type == "word" else f"There are more than {int(count[0])} cows."
    else:  # count is a tuple of finite numbers
        left, right = int(count[0]), int(count[1])
        left, right = num2word(left), num2word(right) if prompt_type == "word" else left, right
        return f"There are between {left} and {right} cows."

    # if count == 0:
    #     return "There is no shrimp." if prompt_type == "word" else "There is 0 shrimp."
    # elif count == 1:
    #     return "There is one shrimp." if prompt_type == "word" else "There is 1 shrimp."
    # elif isinstance(count, (int, float)):
    #     return f"There are {num2word(int(count))} shrimps." if prompt_type == "word" else f"There are {int(count)} shrimps."
    # elif count[1] == float("inf"):
    #     return f"There are more than {num2word(int(count[0]))} shrimps." if prompt_type == "word" else f"There are more than {int(count[0])} shrimps."
    # else:  # count is a tuple of finite numbers
    #     left, right = int(count[0]), int(count[1])
    #     left, right = num2word(left), num2word(right) if prompt_type == "word" else left, right
    #     return f"There are between {left} and {right} shrimps."

    # if count == 0:
    #     return "There is no tree." if prompt_type == "word" else "There is 0 tree."
    # elif count == 1:
    #     return "There is one tree." if prompt_type == "word" else "There is 1 tree."
    # elif isinstance(count, (int, float)):
    #     return f"There are {num2word(int(count))} trees." if prompt_type == "word" else f"There are {int(count)} trees."
    # elif count[1] == float("inf"):
    #     return f"There are more than {num2word(int(count[0]))} trees." if prompt_type == "word" else f"There are more than {int(count[0])} trees."
    # else:  # count is a tuple of finite numbers
    #     left, right = int(count[0]), int(count[1])
    #     left, right = num2word(left), num2word(right) if prompt_type == "word" else left, right
    #     return f"There are between {left} and {right} trees."
